...
Novel Studio for Claude Code는 **13개의 전문 AI 에이전트**가 협업하여 출판급 소설을 자동으로 창작하는 시스템입니다. **Claude Code** 위에서 네이티브로 동작합니다.

`"소설 써줘"` 한마디면 기획, 집필, 편집, 품질 관리까지 에이전트 팀이 모든 것을 처리합니다.

### 💡 왜 Claude Code인가?

다른 CLI 도구는 코드 생성과 터미널 명령에 한정됩니다. **Claude Code는 코딩을 넘어** 소설 창작 같은 창의적 워크플로우를 가능하게 하는 완전한 에이전트 오케스트레이션 플랫폼입니다:

- **Agent Mode** — 단순 Q&A가 아닌 자율적 멀티스텝 실행
- **`AGENTS.md` 자동 로드** — 프로젝트를 여는 순간 시스템이 활성화
- **agent 호출** — 13개 전문가가 이름 참조로 협업
- **Hooks 라이프사이클** — 모든 파일 작업에 자동 품질 체크 실행
- **모델 선택** — Claude Code 설정에서 AI 모델(Claude 등)을 직접 선택하여 속도, 품질, 비용을 조절
- **Claude Code 통합** — 동일한 에이전트 호출과 채팅 기반 워크플로우가 그대로 동작
...
- ✅ **모델 유연성**: Claude 모델을 활용한 최적의 창작 품질
- ✅ **CLI**: Claude Code 터미널에서 동작 — 동일한 에이전트, 동일한 워크플로우

---

## ⚙️ Claude Code 연동 메커니즘

이 프로젝트는 **Claude Code 네이티브** 멀티 에이전트 시스템입니다.

### 1. `AGENTS.md` — 자동 로드 진입점

Claude Code가 프로젝트 루트의 `AGENTS.md`를 **custom_instruction**으로 자동 읽어들입니다. 이 파일이 **영업 담당** 에이전트 역할을 하여, 사용자를 맞이하고 요구사항을 수집하여 전문 에이전트에게 라우팅합니다.

### 2. `.github/agents/*.agent.md` — 전문 에이전트 프로필

13개 에이전트가 각각 `.agent.md` 파일로 정의되어 있습니다. Claude Code가 이 디렉토리를 인식하여 에이전트명 구문으로 호출할 수 있습니다:
...
### 4. 활용한 Claude Code 기능

| 기능 | 활용 방식 |
|------|----------|
| `AGENTS.md` 자동 로드 | 진입점 에이전트 자동 활성화 |
| `.github/agents/` 디렉토리 | 13개 전문 에이전트 이름으로 호출 |
...
프로젝트 디렉토리에서 Claude Code를 실행:

```bash
claude
```

> 💡 이 디렉토리에서 Claude Code가 시작되면 `AGENTS.md`가 자동 로드되어 에이전트 시스템이 활성화됩니다.
...
```
Novel Studio for Claude Code (영업 담당):
  안녕하세요! 몇 가지 확인하겠습니다...
