...
Novel Studio for Claude Code is a system where **13 specialized AI agents** collaborate to automatically create publication-quality novels — all orchestrated natively on **Claude Code**.

Just say `"Write me a novel"` and the agent team handles everything: planning, writing, editing, and quality control.

### 💡 Why Claude Code?

Other CLI tools are limited to code generation and terminal commands. **Claude Code goes far beyond coding** — it provides a full agent orchestration platform that makes creative workflows like novel writing possible:
...
- **Model selection** — choose your preferred AI model (Claude, etc.) directly in Claude Code settings to balance speed, quality, and cost for each writing session
- **Claude Code integration** — the same agent calls and chat-based workflow work seamlessly, giving you a full experience alongside the CLI
...
$ cd novel-studio-claude
  ...
  ┌─────────────────────────────────────────────────────────┐
  │  Novel Studio for Claude Code (Sales Representative)     │
  │                                                         │
...
User: "first_love, review"
  ...
  Novel Studio:
    ...
    🔔 Phase 1 complete. Approve / Revise / Modify?
...
- ✅ **Model Flexibility**: Choose Claude model for optimal creative writing quality
- ✅ **CLI**: Works in Claude Code terminal — same agents, same workflow

---

## ⚙️ How It Works with Claude Code

This project is designed as a **Claude Code-native** multi-agent system. Every component maps directly to a Claude Code feature:

### 1. `AGENTS.md` — Auto-Loaded Entry Point

Claude Code automatically reads `AGENTS.md` from the project root as a **custom instruction**. This file acts as the **Sales Representative** agent — the front door to the entire system. When a user opens Claude Code in this directory, AGENTS.md is injected into the system prompt, enabling the agent to greet the user, gather requirements, and route work to specialized agents.

```
novel-studio-claude/
└── AGENTS.md          ← Claude Code auto-loads this as custom_instruction
```

### 2. `.github/agents/*.agent.md` — Specialized Agent Profiles

Each of the 13 agents is defined as a `.agent.md` file in `.github/agents/`. Claude Code recognizes this directory convention and makes each agent invocable via `agent-name` syntax:
...
Claude Code hooks in the `hooks/` directory trigger automation at key lifecycle events:
...
The system uses a **hierarchical delegation pattern** built entirely on Claude Code's agent invocation:
...
| **`custom_instruction`** | `AGENTS.md` shapes Claude Code behavior for the entire project |
...
### Required
1. **Claude Code** — [Installation Guide](https://docs.anthropic.com/en/docs/claude-code)
   ```bash
...
1. **Clone & Enter**:
   ```bash
   git clone https://github.com/tiny-flowlab/novel-studio-claude.git
   cd novel-studio-claude
   ```
...
Novel Studio:
  ...
  🔔 Phase 1 complete — approval needed. Approve / Revise / Modify?
...
```
novel-studio-claude/
├── AGENTS.md                    ← Sales Representative (auto-loaded by Claude Code)
...
## 📈 Performance: Evolution to v2.0

| Metric | Beta (Manual) | v2.0 (Current) | Improvement |
|--------|---------------|-----------------|-------------|
| Agent Count | 8 | 13 | +5 |
...
|---------|-------|----------|
| Agent not responding | Not running in Claude Code session | Open project directory and run `claude` |
...
- ✅ Claude Code native architecture (AGENTS.md, .github/agents/, hooks)
...
**Novel Studio for Claude Code — by [tiny_flowlab](https://github.com/tiny-flowlab)**
